...
/* NOTE: Verify the correct foreign key column for external user notifications.
 * Current resident version uses resident_id. The older external file used external_complaint_id which seems likely incorrect (points to a complaint record id, not user id).
 * If your notifications table stores external user reference as external_user_id or external_complainant_id, adjust below accordingly.
 */
$externalColumn = 'external_complaint_id'; // adjust if schema differs (e.g., external_user_id / external_complainant_id)
$sql = "SELECT * FROM notifications WHERE $externalColumn = ? ORDER BY created_at DESC";
