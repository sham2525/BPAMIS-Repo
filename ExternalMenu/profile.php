...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_profile'])) {
    ...
    if ($first_name && $last_name && $email) {
        $update_query = "UPDATE external_complainant SET First_Name=?, Middle_Name=?, Last_Name=?, email=?, Contact_Number=? WHERE external_complaint_id =?";
        $stmt = $conn->prepare($update_query);
        $stmt->bind_param("sssssi", $first_name, $middle_name, $last_name, $email, $contact, $user_id);
        if ($stmt->execute()) {
                ...
                <div class="premium-card rounded-2xl p-4 sm:p-6">
                    ...
                    <div class="space-y-3 sm:space-y-4">
                        ...
                        <div class="text-center mt-2 sm:mt-4">
                            <a href="home-external.php#calendar" class="text-blue-600 hover:underline font-semibold text-xs sm:text-base">View More</a>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <form class="space-y-6" method="post" action="">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="middle_name" value="<?= explode(' ', $user_name)[1] ?? '' ?>" class="premium-input w-full px-4 py-3 rounded-lg focus:outline-none">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-2">Last Name</label>
                                            <input type="text" name="last_name" value="<?= explode(' ', $user_name)[2] ?? '' ?>" class="premium-input w-full px-4 py-3 rounded-lg focus:outline-none">
                                        </div>
                                    </div>
                                    <div class="relative">
