...
$stmt->bind_param('ii',$caseId,$externalId);
$stmt->execute();
$res=$stmt->get_result();
...
<body class="bg-gray-50 font-sans relative overflow-x-hidden min-h-screen">
    ...
    <main class="relative z-10 max-w-6xl mx-auto px-4 pt-10 pb-24 animate-fade-in">
        <div class="mb-6 flex items-center gap-3">
            <a href="view_cases.php" class="inline-flex items-center gap-2 text-sm text-primary-700 font-medium hover:text-primary-900 transition"><span class="w-8 h-8 rounded-lg bg-white/70 backdrop-blur flex items-center justify-center shadow"><i class="fa fa-arrow-left"></i></span><span>Back to Cases</span></a>
        </div>
        <section class="relative glass rounded-2xl p-8 md:p-10 border border-white/60 shadow-sm overflow-hidden">
            ...
            <div class="grid gap-10 md:grid-cols-5">
                ...
                <aside class="md:col-span-2 space-y-6">
                    ...
                    <div class="rounded-2xl bg-gradient-to-br <?= ''.$style['gradient'] ?> text-white p-6 shadow-sm">
                        <div class="flex items-center gap-3 mb-3"><div class="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center"><i class="fa fa-gavel text-white text-xl"></i></div><div><p class="text-xs uppercase tracking-wide text-white/70 font-semibold">Current Status</p><p class="text-base font-medium"><?= htmlspecialchars($status) ?></p></div></div>
            ...
            <div class="mt-12 pt-6 border-t border-dashed border-primary-200 flex items-center justify-between flex-wrap gap-4">
                ...
                <div class="flex gap-2"><a href="view_cases.php" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white/80 hover:bg-white text-primary-700 border border-primary-200 shadow-sm text-sm font-medium transition"><i class="fa fa-arrow-left"></i> Back</a></div>
            </div>
        </section>
    </main>
