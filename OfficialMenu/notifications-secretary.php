...
// Fetch all relevant notifications for the secretary
$sql = "SELECT * FROM notifications WHERE type IN ('Unverified', 'Hearing', 'Complaint', 'Case') ORDER BY created_at DESC";
$result = $conn->query($sql);
...
<div id="notification-regular">
  ...
  <div id="notification-unverified" class="w-full mt-6 px-4 pb-10">
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="divide-y divide-gray-100">
            <div class="p-4 bg-gray-50">
                <h3 class="text-sm font-medium text-gray-500">Unverified Accounts</h3>
            </div>

            <?php if (!empty($unverifiedAccounts)): ?>
                <?php foreach ($unverifiedAccounts as $account): ?>
                    <div class="notification-card p-5 relative cursor-pointer">
                        <div class="flex">
                            <div class="flex-shrink-0 mr-4">
                            ...
                            <div class="flex-grow">
                                <p class="text-sm font-medium">Unverified Account</p>
                                ...
                                <div class="flex justify-between items-center mt-2">
                                    ...
                                    <div class="flex gap-2">
                                        <form method="POST" action="../controllers/account_verification.php">
                                            ...
                                            <button name="verify" type="submit" class="text-green-600 hover:text-green-700 text-sm">Verify</button>
                                        </form>
                                        <form method="POST" action="../controllers/account_verification.php">
                                            <input type="hidden" name="id" value="<?php echo $account['resident_id']; ?>">
                                            <button name="remove" type="submit" class="text-red-600 hover:text-red-700 text-sm">Remove</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
