...
<head>
  ...
  <style>
    ...
    .fc .fc-toolbar-title { font-weight:700; font-size:1.15rem; color:#0281d4; letter-spacing:.5px; display:flex; align-items:center; gap:.4em; }
    .fc .fc-button { box-shadow:none!important; padding:.32rem .60rem; border-radius:7px!important; font-weight:600; font-size:.78rem; line-height:1; transition:all .18s; text-transform:capitalize; border:1px solid #b6e1f7!important; background:linear-gradient(120deg,#e8f7ff 0%, #f3fbff 100%)!important; color:#0276c4!important; margin:0 .15rem; display:flex; align-items:center; gap:.25em; min-height:30px; }
    .fc .fc-button .fc-icon { font-size:.9rem; }
    .fc .fc-prev-button, .fc .fc-next-button { padding:.25rem .5rem!important; width:32px; }
    ...
    .fc .fc-daygrid-day-number { padding:12px; font-size:1rem; color:#0281d4; font-weight:600; }
    .fc .fc-daygrid-day.fc-day-today { background:linear-gradient(90deg,rgba(199,229,248,.85) 0%, #f0f9ff 100%)!important; border-radius:12px; box-shadow:0 2px 8px 0 rgba(2,129,212,.08); }
    .fc .fc-event { border:none!important; padding:10px 18px 10px 16px; font-size:1.08rem!important; font-weight:700; margin-top:6px; border-radius:14px!important; background:rgba(214,214,214,.28)!important; color:#1e293b!important; box-shadow:0 4px 16px 0 rgba(2,129,212,.13); display:flex; align-items:center; gap:.7em; position:relative; transition:transform .2s, box-shadow .2s, background .2s; backdrop-filter:blur(2px); overflow:hidden; animation:eventFadeIn .5s; line-height:1.3; white-space:nowrap; text-overflow:ellipsis; }
    .fc .fc-event:hover, .fc .fc-event:focus { transform:scale(1.06) translateY(-3px); box-shadow:0 10px 28px 0 rgba(2,129,212,.22); z-index:2; background:rgba(255,255,255,.38)!important; }
    @keyframes eventFadeIn { from {opacity:0; transform:translateY(10px);} to {opacity:1; transform:translateY(0);} }
    #event-details-modal .bg-white { background:rgba(255,255,255,.95); box-shadow:0 8px 32px 0 rgba(31,38,135,.13); border-radius:22px; border:1.5px solid #e0e7ef; backdrop-filter:blur(8px); }
    #event-details-modal h5 { color:#0281d4; font-weight:700; font-size:1.3rem; }
    #event-details-modal dt { color:#0281d4; font-weight:600; }
    #event-details-modal dd { font-size:1.08rem; }
    #event-details-modal button { transition: background .2s, color .2s, box-shadow .2s; }
    #event-details-modal button:focus { outline:2px solid #0281d4; outline-offset:2px; }
    @media (max-width: 640px) {
      #calendar { padding:.5rem .1rem .5rem .1rem; min-width:0; width:100%; box-sizing:border-box; }
      .fc .fc-toolbar-title { font-size:.95rem; }
      .fc .fc-button { font-size:.7rem; padding:.28rem .5rem; }
      .fc .fc-daygrid-day-number { font-size:.92rem; padding:6px; }
      .fc .fc-event { font-size:.95rem!important; padding:8px 4px 8px 8px; flex-direction:row; align-items:center; min-width:0; max-width:100%; white-space:nowrap; text-overflow:ellipsis; }
      .fc .fc-event-title { color:#1e293b!important; font-size:.95rem; }
    }
    ...
    .fc-listWeek-view .fc-list-day.fc-day-today { background:linear-gradient(90deg,rgba(199,229,248,.85) 0%, #f0f9ff 100%)!important; }
    ...
    .fc-timeGridDay-view .fc-timegrid-slot-label.fc-day-today { background:linear-gradient(90deg,rgba(199,229,248,.85) 0%, #f0f9ff 100%)!important; }
  </style>
</head>
  ...
  <div class="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center hidden" id="event-details-modal">
    <div class="bg-white rounded-lg w-full max-w-md">
      ...
      <div class="border-t px-6 py-4 flex justify-end space-x-2">
        ...
        <!-- Delete button intentionally removed for Captain -->
    ...
    $(function () {
      ...
      calendar = new Calendar(document.getElementById('calendar'), {
        ...
        eventClick: function (info) {
          ...
          // Captain should view OfficialMenu case details
          $('#view-details-link').attr('href', '../../OfficialMenu/view_case_details.php?id=' + event.Case_ID);
      ...
      // Reschedule redirects to OfficialMenu page
      $('#reschedule').click(function () {
        const id = $(this).attr('data-id');
        window.top.location.href = `../../OfficialMenu/reschedule_hearing.php?id=${id}`;
      });
    });
