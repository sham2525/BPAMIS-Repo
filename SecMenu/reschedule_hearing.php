...
<head>
  ...
  <style>
    /* Mimic disabled select style but keep enabled */
    select.readonly-style {
      background-color: #e0effe !important;
      color: #0c9ced !important;
      cursor: not-allowed;
 ...
 document.addEventListener('DOMContentLoaded', () => {
    ...
    if (hearingIdFromUrl && dropdown) {
        ...
        if (optionExists) {
            ...
            // Add readonly styling class
            dropdown.classList.add('readonly-style');

            // Disable the dropdown to prevent changes (readonly)
            dropdown.setAttribute('disabled', 'disabled');

            // Trigger change event to load the data for the selected hearing
